...
int BinomialHeap::Display(node* H)
{
    ...
    unordered_map<int, vector<node*>> binomialTrees;

    node* p = H;
    while (p != NULL)
    {
        binomialTrees[p->degree].push_back(p);
        p = p->sibling;
    }

    for (const auto& entry : binomialTrees)
    {
        cout << "Binomial Tree, B" << entry.first << endl;
        for (node* nodePtr : entry.second)
        {
            queue<pair<node*, int>> q;
            q.push(make_pair(nodePtr, 0));

            while (!q.empty())
            {
                pair<node*, int> currentPair = q.front();
                q.pop();

                node* current = currentPair.first;
                int level = currentPair.second;

                cout << "Level " << level << " : " << current->n << endl;
   ...
   int BinomialHeap::Revert_list(node* y)
{
    ...
    y->sibling = NULL; // Add this line to correctly reset sibling pointers
}
