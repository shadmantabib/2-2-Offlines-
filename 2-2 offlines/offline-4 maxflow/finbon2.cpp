...
int cap[1000][1000];
vector<int> adj[1000];
int n, wins[1000], losses[1000], remaining[1000], games_played[1000][1000];
...
*/
            ...
            cout << team_names[(marker+1)%2] << " has won a total of " << max_wins << " games"<<endl;
    ...
    for (int team = 0; team < n; team++)
    {
        ...
        if (flag == 1)
        {
            ...
            if (max_wins > wins[team] + remaining[team])
            {
                ...
                cout << "They play each other " << 0 << " times" << endl;
                cout << "So on average, each of the teams in this group wins " << max_wins << "/" << 1 << "=" << max_wins / 1 << " games"<<endl;
            }
            else
            {
                queue<int> q;
