...
int solve(int testcase)
{
    ...
    // Perform BFS to find the nodes reachable from the source (n-2) in the residual graph.
    vector<bool> visited(n, false);
    queue<int> q;
    q.push(n - 2); // Push the source node into the BFS queue.
    visited[n - 2] = true; // Mark the source as visited.
    ...
    while (!q.empty())
    {
        ...
        // Explore the adjacent vertices of the current node.
        for (int v : adj[u])
        {
    ...
    // Output the vertices in the 's' part (source side of the minimum cut)
    cout << "Vertices in the 's' part (Maximum Matching): ";
    for (int i = 0; i < n; i++) {
        if (visited[i]) {
            cout << i << " ";
        }
