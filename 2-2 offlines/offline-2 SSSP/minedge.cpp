...
struct node {
    ...
    int cost;
    vector<int> path;
    int numEdges; // Number of edges in the path

    // Comparison function for the priority queue
    bool operator<(const node& other) const {
        if (cost == other.cost)
            return numEdges > other.numEdges; // Choose the path with fewer edges
        return cost > other.cost; // Choose the path with smaller cost
    }
};
    ...
    while (!PQ.empty()) {
        ...
        for (int i = 0; i < G[val].size(); i++) {
            ...
            if (vis[nxt] == 0) {
                ...
                PQ.push({nxt, cost + nxtCost, newPath, numEdges + 1});
            }
        }
    }
