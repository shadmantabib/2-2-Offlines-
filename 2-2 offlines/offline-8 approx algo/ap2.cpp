...
public:
    ...
    // Add any other necessary functions for your graph representation
};
...
vector<pair<int, int>> DoubleMSTEdges(const vector<pair<int, int>> &mstEdges)
{
    vector<pair<int, int>> eulerianEdges;
...
int Metric_Approximation_TSP(Graph G)
{
    ...
    vector<pair<int, int>> eulerianEdges = DoubleMSTEdges(mstEdges);

    // Step 3: Find an Eulerian tour on this graph
    vector<int> eulerianTour = FindEulerianTour(eulerianEdges, n);

    // Step 4: Let C be the tour that visits vertices of G in the order of their first appearance in τ
    ...
    // Step 5: Return this tour C
    int tourLength = Calculate_Tour_Length(tourC, G);

    // Print the Metric Approximation TSP Tour
...
int main()
{
    ...
    for (int x = 1; x <= X; x++)
    {
        ...
        double ratio = static_cast<double>(approxTourLength) / optimalTourLength;
        cout << "(" << x << "," << ratio << ")" << endl;
    }
